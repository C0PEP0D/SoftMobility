...
\begin{figure}[t]
\begin{center}
    \includegraphics[scale=0.66]{./Figures/Fig4}
\caption{
...
\begin{figure}[t]
\begin{center}
    \includegraphics[scale=0.66]{./Figures/Fig5}
\caption{
	Sinking dynamics of a flexible fiber ($N=10$ beads of radius $a$) The torsional spring stiffness is $k=K_b/2a$, with $K_b$ the equivalent beam rigidity and dimensionless rigidity $\beta = K_b / F_\perp L^2$, with $F_\perp = N m g$ the total weight.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Rotating flexible fiber}
...
\begin{figure}[t]
\begin{center}
    \includegraphics[scale=0.66]{./Figures/Fig6}
\caption{
	Shape of a rotating flexible filament. 
	The filament is made of $N=8$ beads and is set at an angle $\psi=15^\circ$ with an offset of $\delta_0/L=0.05$ with respect to the horizontal rotating axis. 
	The only dimensionless number is the Sperm number  
	$\mathrm{Sp} = {L}/{L_\omega}$, with
	$L_\omega = \left({K_b}/{\mu_\perp\,\omega}\right)^{1/4}$ and
	$\mu_\perp = {4\pi\mu}/{\ln(L/a) + 1/2}$, the sleder-body transverse drag.
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Jeffery orbits for a rigid body}

\cite{Jeffery1922,Bretherton1962,Thorp2019,Ishimoto2023}

%%%%%%%%%%%%%%%%%
\begin{figure}[t]
\begin{center}
    \includegraphics[scale=0.66]{./Figures/Fig7}
\caption{
	Jeffery orbits of a rigid dumbbell in a pure shear flow of unit shear rate in the $\be_1$-$\be_2$ plane: $\bu^\infty_0 = [e_2, 0, 0]$.
	...
	(c) Example of an orbit of $\bE_1$ as a function of time in the spherical coordinate $(r, \theta, \phi)$.	
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Optimisation of a three-sphere swimmer}
...
\begin{figure}[t]
\begin{center}
    \includegraphics[scale=0.66]{./Figures/Fig8}
\caption{
	Optimization of the three-sphere swimmer.
	(a) Default geometry with $L_2 = l_2(1+\varepsilon\sin(\omega t))$, spring rest length $l_1=l_2$, and sphere radius $a_i=0.05 l_2$.
	(b) Optimized geometry for fixed $a_0=a_2=0.05$.
	(c) Orbits of the swimmer in the $L_2$-$L_1$ plane for different values of the dimensionless frequency $\Omega = \omega \mu (l_1 + l_2) / k$, with $k$ the spring stiffness. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Jeffery orbits for a soft body}
