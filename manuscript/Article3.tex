...
Its appeal, from the standpoint of the inverse problem, is the explicit, shape-dependent linear relation it provides between velocities and loads~\cite{Moreau2023}.
...
The material is assumed hyperelastic, so that the second Piola–Kirchhoff stress tensor $\tS$ derives from a strain energy density $W(\tE)$ as $\tS = \partial W/\partial\tE$, where $\tE = (\tF^T\cdot\tF - \tI)/2$ is the Lagrangian Green strain tensor. The outward unit normals to the body surface in the reference and deformed configurations are denoted $\bN(\bX)$ and $\bn(\bx)$, respectively. 
...
\int_{\partial \Omega_0} \bF_s^* \cdot \bxd \, \rmd S =
	\int_{\partial \Omega_0} \bF_s^* \cdot  \bu^\infty \, \rmd S + 
	\int_{\Omega_0} \tS : \tEd^* \, \rmd V -
	\int_{\Omega_0} \bF_v \cdot \bV^* \, \rmd V,
\end{equation}
...
\end{equation}
where $\tU(\br)$ is the transport tensor of Eq.~(\ref{eq:bg_flow}) and $\bV_\mathrm{def}$ is the velocity due to shape change at fixed $\bv_0$.

Two structural consequences follow. First, for any rigid-body virtual velocity $\bV^* = \tU(\bchi-\br_0)\cdot\bv_0^*$, the hyperelastic term in Eq.~(\ref{eq:weak_form}) vanishes. This is because  $\tF^T \cdot \nabla \bV^*$ is then skew-symmetric in Eq.~(\ref{eq:virtual_strain_rate}) when $\nabla\bV^* = [\bomega_0^*]_\times\cdot\tF$.
Second, comparing Eq.~(\ref{eq:velocity_decomp}) with the background flow linearization~(\ref{eq:bg_flow}), the same transport operator $\tU(\bchi-\br_0)$ couples $\bv_0$ on the left of Eq.~(\ref{eq:weak_form}) and $\bv_0^\infty$ on the right. Only the relative velocity $\bv_0 - \bv_0^\infty$ therefore drives the hydrodynamic coupling, consistent with Galilean invariance.
...
Equation~(\ref{eq:weak_form}) is a weak formulation on infinite-dimensional space $V$. Because the background flow $\bu^\infty$ is linear in $\bv_0^\infty=[\bu_0^\infty, \bomega_0^\infty]$ and $\tE_0^\infty$ [Eq.~(\ref{eq:bg_flow})], and the problem is linear in $\bxd$, the solution decomposes into contributions driven by each input separately.
...
\end{equation}
Testing Eq.~(\ref{eq:weak_form}) with each $\bV_i$ and using the linearity in $\bv_0^\infty$ and $\tE_0^\infty$ yields the projected system
...
\end{equation}
where $\ba = [a_1,\dots a_n]$ and $\tE_0^\infty$ is the $3\times3$ symmetric traceless rate-of-strain tensor, which in practice is represented as a 5-component vector (see Appendix~\ref{app:strain_vec}). All tensors depend on the current configuration $\bchi(\bX,t)$. The resistance matrix $\hat{\tR}$ is symmetric positive definite by virtue of the reciprocal theorem. The nonlinear stiffness $\hat{\tK}_{NL}$ is symmetric positive semi-definite by its relation to the elastic energy.
...
We consider an assembly of $N$ spheres in a body frame $(\bE_1,\bE_2,\bE_3)$ attached to the assembly. The $i$-th sphere of radius $a_i$ has center position $\bR_i(\bQ,t)$ and Rodrigues orientation vector $\bTheta_i(\bQ,t)$ in the body frame, both functions of time and the $N_Q$ degrees of freedom $\bQ$ representing the deformation (Fig.~\ref{fig:assembly_spheres}). Its six-component velocity $\bV_i = [\bU_i, \bOmega_i]$ is related to $\bQd$ by the per-sphere Jacobian $\tJ_i = \partial \bV_i/\partial \bQd$ (see Appendix~\ref{app:bortz} for the Bortz operator relating angular velocity $\bOmega_i$ to the orientation rate $\bThetad_i$). \new{More generally, allowing $\bR_i$ and $\bTheta_i$ to depend explicitly on time as well as on $\bQ$ --- representing prescribed active kinematics --- the chain rule applied to $\bX_i(\bQ,t) = [\bR_i,\bTheta_i]$ yields the affine relation $\bV_i = \tJ_i \cdot \bQd + (\bv_\mathrm{act})_i$, with $(\bv_\mathrm{act})_i = \tB_i \cdot \partial\bX_i/\partial t$.}
...
\bV = \tJ_\mathrm{sph} \cdot \bQd \new{{} + \bv_\mathrm{act}},
\end{equation}
with the assembly Jacobian $\tJ_\mathrm{sph}(\bQ,t) = \partial \bV/\partial \bQd$ of size $6N \times N_Q$\new{, and the grand active velocity $\bv_\mathrm{act} = [(\bv_\mathrm{act})_1, \dots, (\bv_\mathrm{act})_N]$ collecting the per-sphere contributions. Because $\bV$ is affine in $\bQd$, it is a quasi-velocity of the Lagrangian system}.
...
In the laboratory frame, the assembly also undergoes a global rigid-body motion: translation $\bu_0$ and rotation $\bomega_0$ of the body frame, encoded in the six-component grand velocity $\bv_0 = [\bu_0, \bomega_0]$ and the corresponding generalized coordinate $\bx_0 = [\br_0, \btheta_0]$, where $\br_0$ is the position and $\btheta_0$ the Rodrigues orientation of the body frame. The full state of the assembly in the laboratory frame is therefore the $N_q = 6 + N_Q$ generalized coordinate
$$\bq = \left[\bx_0,\, \bQ \right]$$
and the associated generalized velocity
$$\bp = \left[\bu_0,\, \bomega_0,\, \bQd \right] = \tB \cdot \bqd,$$
where $\tB$ is the block-diagonal Bortz matrix assembled from the per-degree-of-freedom operators of Appendix~\ref{app:bortz}.
...
\bv = \new{\bv_\mathrm{act} + {}}\tJ \cdot \bp = \new{\bv_\mathrm{act} + {}}\tJ \cdot \tB \cdot \bqd,
\end{equation}
...
The linearized background flow [Eq.~(\ref{eq:bg_flow})] evaluated at the sphere centers gives the grand flow velocity $\bv^\infty = [\bu^\infty(\br_i)]_{i=1}^N$ and the rate-of-strain tensor
\begin{equation}\label{eq:grandflowvelocity}
\bv^\infty = \tC_U \cdot \bv_0^\infty + \tC_S : \tE_0^\infty, \quad \tE^\infty = \tE_0^\infty,
\end{equation}
where $\tC_U$ is the same transport coupling tensor as in Eq.~(\ref{eq.composition_velocities}) and $\tC_S$ is a $6N\times 3\times 3$ stresslet coupling tensor. Here $\bv_0^\infty = [\bu_0^\infty, \bomega_0^\infty]$ denotes the six-component velocity of the undisturbed flow evaluated at the reference point $\br_0$.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Soft mobility equation of a sphere assembly}

With the kinematic description established, we derive the equation of motion by applying the principle of virtual power to the force balance on the sphere assembly, then reduce it to a closed-form soft mobility equation.

\paragraph{Force balance.}
The six-component force $\bff_i = [\bF_i, \bT_i]$ acting on sphere $i$ collects the force $\bF_i$ applied at its center and the associated torque $\bT_i$. Stacking all spheres, the total grand force decomposes as
$$\bff = \bff_c + \bff_\mathrm{hydro} + \bff_\mathrm{ext} + \bff_K,$$
where $\bff_c$ are internal constraint forces (maintaining rigidity of each sphere and prescribed inter-sphere constraints), $\bff_\mathrm{hydro}$ are hydrodynamic forces, $\bff_\mathrm{ext}$ are external body forces, and $\bff_K$ are elastic restoring forces. The elastic forces are linear in the deformation state,
\begin{equation}\label{eq.linear_elasticity}
\bff_K = \tC_K \cdot \bQ,
\end{equation}
and satisfy $\sum_{i=1}^N (\bff_K)_i = 0$ (Newton's third law for internal forces). The external forces are linear in an external field $\bH$,
\begin{equation}\label{eq.body_force}
\bff_\mathrm{ext} = \tC_H \cdot \bH.
\end{equation}

The principle of virtual power eliminates the unknown constraint forces. Any virtual velocity $\bv^*$ compatible with the constraints can be written $\bv^*_i = \sum_j (\partial \bv_i/\partial p_j)\,p_j^*$, so the virtual power of all forces except constraints vanishes:
$$P^* = \sum_i \left[(\bff_\mathrm{hydro})_i + (\bff_\mathrm{ext})_i + (\bff_K)_i\right] \cdot \bv^*_i = 0 \quad \forall\,\bp^*.$$
Since this must hold for all $\bp^*$, one obtains
\begin{equation}\label{eq.generalized_force_Q}
\tJ^T \cdot \left(\bff_\mathrm{hydro} + \bff_\mathrm{ext} + \bff_K\right) = 0,
\end{equation}
where $\tJ = \partial \bv/\partial \bp$ is the Jacobian of Eq.~(\ref{eq.U_is_J_Qdot}). This projection reduces the $6N$-dimensional force balance to the $N_q$ generalized coordinates while eliminating the constraint forces $\bff_c$ from the equations of motion.
...
\bp & = & \tPi \cdot \left[ \tR^{-1} \cdot \bff \new{{} - \bv_\mathrm{act}} + \tC_U \cdot \bv_0^\infty + (\tC_S + \tR^{-1} \cdot \tR_S) : \tE_0^\infty \right],\\
\bp & = & \tM \cdot \bff \new{{} - \tPi \cdot \bv_\mathrm{act}} + \tC_V \cdot \bv_0^\infty + \tC_E : \tE_0^\infty,\label{eq:soft_mobility}
\end{eqnarray}
upon introducing the $N_q\times 6N$ projection operator
\begin{equation}\label{eq:projection}
\tPi = \left( \tJ^T \cdot \tR \cdot \tJ \right)^{-1} \cdot \tJ^T \cdot \tR,
\end{equation}
which satisfies $\tPi \cdot \tJ = \tI_{N_q}$ and eliminates the constraint forces. The soft mobility tensor and reduced flow-coupling tensors are
\begin{equation}
\tM = \tPi \cdot \tR^{-1} = \left(\tJ^T \cdot \tR \cdot \tJ\right)^{-1} \cdot \tJ^T, \quad
\tC_V = \tPi \cdot \tC_U, \quad
\tC_E = \tPi \cdot (\tC_S + \tR^{-1} \cdot \tR_S).
\end{equation}
The tensor $\tM$ ($N_q\times 6N$) extends the classical rigid-body mobility to deformable assemblies. The tensor $\tC_V$ ($N_q\times 6$) captures the response to the background rigid-body velocity; its top $6\times 6$ block equals the identity, reflecting that the assembly's own rigid-body motion follows the background flow in the absence of forces. Substituting the force decompositions~(\ref{eq.linear_elasticity})--(\ref{eq.body_force}) yields
...
\end{array}
\right) =
  \tM_K \cdot \bQ + \tM_H \cdot \bH + \tC_E : \tE_0^\infty \new{{} - \tPi \cdot \bv_\mathrm{act}},
\end{equation}
where $\tM_K$, $\tM_H$, \new{$\tPi$,} and $\tC_E$ depend only on the deformation state $\bQ$. Since $\bp = \tB^{-1}\cdot\bqd$, this is a first-order ODE $\bqd = f(\bq, t)$ that can be integrated forward in time given initial conditions and the prescribed flow and external fields. \new{An apparent slip velocity $\bv_\mathrm{slip}$ between the fluid and the spheres can equivalently be modeled as an additional external force $\bff_\mathrm{slip} = \tR \cdot \bv_\mathrm{slip}$ entering through the $\tM\cdot\bff$ term of Eq.~(\ref{eq:soft_mobility}), so it does not require a dedicated tensor.}
...
\bE = [E_{xx},\, E_{xy},\, E_{xz},\, E_{yy},\, E_{yz}],
\end{equation}
...
The double contractions $\hat{\tC}_E:\tE_0^\infty$ and $\tC_E:\tE_0^\infty$ appearing in Eqs.~(\ref{eq:galerkin_system}) and (\ref{eq:soft_mobility}) can each be expressed as a matrix-vector product with $\bE$. Because every entry $(\tE_0^\infty)_{jk}$ is a linear combination of the five components $E_l$ [Eq.~(\ref{eq:strain_mat})], the double contraction
\begin{equation}
\left(\tC_E:\tE_0^\infty\right)_i = \sum_{j,k}(C_E)_{ijk}\,(\tE_0^\infty)_{jk}
\end{equation}
is linear in $\bE$, so it can be written as $\tC_E^{(5)}\cdot\bE$ for an equivalent $N_q\times 5$ matrix $\tC_E^{(5)}$. The entries of $\tC_E^{(5)}$ are obtained by substituting Eq.~(\ref{eq:strain_mat}) and collecting coefficients of each $E_l$. Because the sum runs over all nine $(j,k)$ pairs and $\tE_0^\infty$ is symmetric, each off-diagonal component (e.g.\ $E_{xy}$) contributes through both $(j,k)=(1,2)$ and $(j,k)=(2,1)$, automatically yielding the correct factor of two without requiring explicit prefactors in $\bE$.
