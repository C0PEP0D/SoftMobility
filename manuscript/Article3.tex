...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Jeffery's orbits for a rigid body}
...
\end{equation}
and in a shear flow $\bu^\infty = \dot\gamma\,e_2\,\be_1$, the angular velocity and rate-of-strain tensor reduce to $\bomega^\infty =- \dot\gamma\be_3/2 $ and $\tE^\infty = \dot\gamma(\be_1 \otimes \be_2 + \be_2 \otimes \be_1)$. 
...
orbit constant of motion $K$, which is set b the initial condition.  
...
\caption{
	Jeffery orbits of a rigid dumbbell in a pure shear flow of unit shear rate in the $\be_1$-$\be_2$ plane: $\bu^\infty = [e_2, 0, 0]$.
...
In several applications one prescribes the velocity of a single sphere of the assembly --- the \emph{anchor} --- rather than letting the assembly translate and rotate freely under external forces. Examples include tethered fibers, the rotating elastic fiber of \cite{Coq2008,Coq2009}, and kinematically actuated swimmers. We extend here the soft mobility equation~(\ref{eq:soft_mobility_body}) to such a clamped-anchor problem.
