...
The principle of virtual power allows us to eliminate the unknown constraint forces. For any virtual velocity $\bv^*$ compatible with the constraints, the constraint forces do not work and hence $\bff_c \cdot \bv^* = 0$.   so the virtual power of all forces except constraints vanishes:
$$P^* = \sum_i \left[(\bff_\mathrm{hydro})_i + (\bff_\mathrm{ext})_i + (\bff_K)_i\right] \cdot \bv^*_i = 0, \quad \forall\,\bp^*.$$

can be written $\bv^*_i = \sum_j (\partial \bv_i/\partial p_j)\,p_j^*$,
